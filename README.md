...
4. Two while loops handle the expansion:
    a. The first while loop handles the case where the potential palindrome has an odd length (centered at a single character).
    b. The second while loop handles the case where the potential palindrome has an even length (centered between two characters).
